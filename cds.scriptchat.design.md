...
- **Scope was narrowed on 2026-08-24 (D21): C# script chat, nothing else.**
  ...
  defensible and the sum was not. Jobs 6 (host-registered tools) and 7 (settings
  chat) were parked, then deleted from the backlog on 2026-09-24; their write-ups
  are in git history (`git log -p -- todo.features.md`). **Read D21 and D22 before reopening any of it.**
...
| D24 | **Two packages, `CDS.ScriptChat.Core` and `CDS.ScriptChat.WinForms`, always released together on one MinVer version.** Reaffirmed 2026-08-24 after explicitly considering a merge. The case for merging: nobody installs Core directly — every consumer references `CDS.ScriptChat.WinForms` and gets Core transitively — so the split is invisible at install time. Rejected because the cost is real (moving ~26 files, a namespace choice that either confuses or breaks every consumer's `using CDS.ScriptChat.Core;`, retargeting `Core.Tests` to `net10.0-windows`, reworking `release.yml`, deprecating a live nuget.org package) and the gain is one fewer package name. The split also earns its keep as a fence: Core cannot reference WinForms, so engine logic cannot drift into the panel. Revisit only if a headless, console or non-WinForms consumer appears. "Room for a future WPF/Avalonia panel" is *not* the reason — rejected as speculative. |
...
- **UC7 (streaming)**, **multi-script host support**, and **local/self-hosted
  provider support** (local providers are now issue #38),
...
- **An `AddScript` overload taking a live compilation source.** A host whose
  ...
  Playground migration is the test of whether it is worth it.
...
- **Local/self-hosted provider** (Ollama, LM Studio, llama.cpp): no base-URL
  override exists today; tracked as issue #38. Realistically
